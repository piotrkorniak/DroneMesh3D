...
Architektura opiera się na czystej separacji między frontendem Angular 21 (renderowanie mapy, interakcje rysowania, walidacja po stronie klienta) a backendem .NET 10 z MediatR (odbiór API → command/query → handler → persystencja). Backend składa się z dwóch projektów: `Api` (warstwa HTTP, endpointy, DTOs, MediatR pipeline) i `Core` (logika biznesowa, encje, dane, interfejsy). Api referencjonuje Core. Obie warstwy wymuszają te same reguły walidacji, aby zapewnić integralność danych.
...
Serwis klienta HTTP z inject() (Angular 21). **Generowany automatycznie z OpenAPI spec backendu** za pomocą `openapi-generator` — nie piszemy ręcznie interfejsów HTTP. Backend eksponuje `/openapi/v1.json`, a frontend generuje z niego typesafe klienta.

```typescript
// Generated by openapi-generator from backend's OpenAPI spec
...
// Core/Models/GeoJsonGeometry.cs
...
// Api/DTOs/ErrorResponse.cs
namespace DroneMesh3D.Api.DTOs;

public record ErrorResponse(string Message);

// Api/DTOs/ValidationErrorResponse.cs
...
```csharp
// Core/Validation/AreaValidator.cs
...
```csharp
// Core/Repositories/AreaRepository.cs
...
```csharp
// Core/Interfaces/IAreaValidator.cs
...
// Core/Interfaces/IAreaRepository.cs
...
WORKDIR /src
COPY Api/DroneMesh3D.Api.csproj Api/
COPY Core/DroneMesh3D.Core.csproj Core/
RUN dotnet restore Api/DroneMesh3D.Api.csproj
COPY Api/ Api/
COPY Core/ Core/
RUN dotnet publish Api/DroneMesh3D.Api.csproj -c Release -o /app/publish
...
services:
  ...
  api:
    ...
    volumes:
      - ./Api:/src/Api
      - ./Core:/src/Core
    command: dotnet watch run --project Api/DroneMesh3D.Api.csproj --urls http://0.0.0.0:8080
    environment:
      - DOTNET_USE_POLLING_FILE_WATCHER=1
```

Przy developmencie wystarczy `docker compose up` — frontend z hot-reload, API z `dotnet watch`, PostgreSQL gotowy. W produkcji budujemy multi-stage images (zoptymalizowane, bez SDK).
...
jobs:
  ...
  test:
    ...
    steps:
      ...
      - name: Build solution
        run: dotnet build DroneMesh3D.slnx --configuration Release

      - name: Run .NET tests
        run: dotnet test DroneMesh3D.slnx --configuration Release --no-build --logger trx
        env:
  ...
  build:
    ...
    steps:
      ...
      - name: Build API image
        run: docker build -t dronemesh3d-api:${{ github.sha }} ./Api

      - name: Build Web image
        run: docker build -t dronemesh3d-web:${{ github.sha }} ./Web
```
