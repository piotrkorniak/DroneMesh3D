...
- **Infrastruktura:** Docker Compose (3 kontenery: Web/nginx, Api/.NET, db/PostgreSQL+PostGIS), hot-reload w development
...
- [x] 1. Konfiguracja struktury projektu, Docker i modeli danych
  - [x] 1.1 Utworzenie struktury Docker Compose
    - Utworzenie `docker-compose.yml` z trzema serwisami: `web` (Angular/nginx), `api` (.NET 10), `db` (PostgreSQL 18 + PostGIS)
    - Utworzenie `docker-compose.override.yml` dla development (hot-reload, volume mounts dla Api/ i Core/)
    - Utworzenie `Api/Dockerfile` (multi-stage: sdk → aspnet, build context to root repo, kopiuje Api/ i Core/ csproj)
    - Utworzenie `Web/Dockerfile` (multi-stage: node → nginx)
    - Utworzenie `Web/Dockerfile.dev` (node z npm start)
    - Utworzenie `Web/nginx.conf` (reverse proxy /api/ → api:8080)
  ...
  - [x] 1.4 Utworzenie projektu Angular 21 z zależnościami OpenLayers
    - Zainicjalizowanie aplikacji Angular 21 w katalogu `Web/` (standalone bootstrap, zoneless z `provideZonelessChangeDetection()`)
    - Zainstalowanie pakietów `ol` (OpenLayers)
    - Skonfigurowanie `angular.json` z globalnym CSS dla OpenLayers (nazwa projektu: `dronemesh3d-web`)
  ...
  - [x] 1.5 Utworzenie projektu .NET 10 Web API z MediatR, EF Core i typami przestrzennymi
    - Utworzenie dwóch projektów: `Api/` (warstwa HTTP) i `Core/` (logika biznesowa, dane, encje). Api referencjonuje Core.
    - Pakiety w `Core/`: `NetTopologySuite`, `Npgsql.EntityFrameworkCore.PostgreSQL`, `Npgsql.EntityFrameworkCore.PostgreSQL.NetTopologySuite`
    - Pakiety w `Api/`: `MediatR`, `FluentValidation.DependencyInjectionExtensions`, `OneOf`, `Scalar.AspNetCore`, `Microsoft.AspNetCore.OpenApi`, `Microsoft.EntityFrameworkCore.Design`
    - Konfiguracja OpenAPI + Scalar w Program.cs (`AddOpenApi`, `MapOpenApi`, `MapScalarApiReference`)
    - Utworzenie encji `AreaEntity` w `Core/Entities/` (sealed class, Id Guid, CreatedAt DateTimeOffset, required Geometry NTS Geometry)
    - Utworzenie `AppDbContext` w `Core/Data/` z primary constructor, konfiguracją PostGIS (`HasPostgresExtension("postgis")`) i typem `geometry(Polygon, 4326)`
    - Utworzenie DTO jako records w `Api/DTOs/`: `CreateAreaRequest`, `AreaResponse`, `ErrorResponse`, `ValidationErrorResponse`
    - Utworzenie modeli w `Core/Models/`: `GeoJsonGeometry`
    - Utworzenie interfejsów w `Core/Interfaces/`: `IAreaValidator`, `IAreaRepository`
    - Konfiguracja `Program.cs`: MediatR + ValidationBehavior + FluentValidation + EF Core (Npgsql + UseNetTopologySuite) + CORS + OpenAPI/Scalar (używa `DroneMesh3D.Core.Data`)
    - Rozwiązanie `DroneMesh3D.slnx` z folderami: Backend (Api+Core), Frontend (Web), Tests
    - _Wymagania: 5.1, 6.1, 6.2, 6.3_

  - [x] 1.6 Utworzenie migracji bazy danych dla tabeli Areas
